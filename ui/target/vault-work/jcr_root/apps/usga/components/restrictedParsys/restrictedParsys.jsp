...
%><%@ include file="/apps/usga/global.jspx" %><%
    ...
    Session ses = resource.getResourceResolver().adaptTo(Session.class);

    boolean hasColumns = false;
    for (Paragraph par: parSys.paragraphs()) {
        boolean allowed = false;
        String childType = par.getResourceType();

        if (request.getAttribute("restrictedParsysAllowedChildren") instanceof String[]) {
            for (String val: (String[]) request.getAttribute("restrictedParsysAllowedChildren")) {
                String pattern = val.replaceAll("\\*", ".*");
                if (childType.matches(pattern)) {
                    allowed = true;
                }
            }

            if(!allowed) {
                Node child = par.adaptTo(Node.class);
                child.remove();
    ...
    request.removeAttribute("restrictedParsysAllowedChildren");
