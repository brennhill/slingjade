...
%><%
    boolean isVanityRedirect = Boolean.parseBoolean(properties.get("vanityRedirect", "false"));
    String path = slingRequest.getRequestURI();
    // resolve variables in path
    path = ELEvaluator.evaluate(path, slingRequest, pageContext);
    String pagePath = currentPage.getPath() + ".html";
    if (path.length() > 0 && !StringUtils.equals(pagePath, path) && isVanityRedirect) {
       %><cq:include script="vanityRedirect.jsp"/><%
    } else { %>
